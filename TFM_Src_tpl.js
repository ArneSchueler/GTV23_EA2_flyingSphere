var app = ( function() {
	...
	function initModels() {
		...
		createModel("torus", fs, [1.6, -0.4, -0], [0, 0, 0], [3, 3, 3]);
		createModel("plane", "wireframe",  [0, 0, 0], [0, 0, 0], [1, 1, 1]);
		//back right
		createModel("sphere", fs, [-0.13, -.3, 1.49], [0, 0, 0], 
            [.5, .5, .5]);
        //back left
        createModel("sphere", fs, [-1.49, -.3, -0.13], [0, 0, 0], 
            [.5, .5, .5]);
        //front right        
        createModel("sphere", fs, [0.13, -.3, -1.49], [0, 0, 0], 
            [.5, .5, .5]);
		...
		window.onkeydown = function(evt) {
			...
			switch(c) {
                ...
                case('Z'):
                    ...
                    break;
                case('K'):
                	 circularMotionAngle += sign * deltaRotate; // Adjust the angle increment as needed

	                // Update the positions of the spheres in a circular path
	                for (var i = 2; i < models.length; i++) {
	                    var radius = 1.5; // Adjust the radius as needed
	                    var xPos = Math.cos(circularMotionAngle + (i - 1) * (Math.PI / 2)) * radius;
